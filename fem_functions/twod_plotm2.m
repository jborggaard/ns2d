function twod_plotm2(n_fig,x,e_conn,symbol,col,lSize)
%-----------------------------------------------------------------------
    ...
    elseif(ndof==4)
      for i=1:n_elems
        plot([x(e_conn(i,1),1), x(e_conn(i,2),1), x(e_conn(i,3),1),...
              x(e_conn(i,4),1), x(e_conn(i,1),1)],...
             [x(e_conn(i,1),2), x(e_conn(i,2),2), x(e_conn(i,3),2),...
              x(e_conn(i,4),2), x(e_conn(i,1),2)],'k')
      end
    elseif(ndof==3)
      for i=1:n_elems
    ...
    if (ndof==6)
      for i=1:n_elems
        plot([x(e_conn(i,1),1), x(e_conn(i,4),1), x(e_conn(i,2),1),...
             ...
             [x(e_conn(i,1),2), x(e_conn(i,4),2), x(e_conn(i,2),2),...
              x(e_conn(i,5),2), x(e_conn(i,3),2), x(e_conn(i,6),2),...
              x(e_conn(i,1),2)],col,'LineWidth',lSize)
      end
    elseif(ndof==3)
      for i=1:n_elems
        plot([x(e_conn(i,1),1), x(e_conn(i,2),1), ...
              x(e_conn(i,3),1), x(e_conn(i,1),1)],...
             [x(e_conn(i,1),2), x(e_conn(i,2),2), ...
              x(e_conn(i,3),2), x(e_conn(i,1),2)],col,'LineWidth',lSize)
      end
    end
  end
